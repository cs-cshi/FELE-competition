...
class Worker(object):
    ...
    def build_projects(self):
        ...
        projects.append(Project(self.booster_dim, self.bin_num, self.feature_num, self.data_bin[:100, :], self.y[:100], self.u))
        #s = random.sample((range(49500)[29500:]), 100)
        projects.append(Project(self.booster_dim, self.bin_num, self.feature_num, self.data_bin[49400:49500, :], self.y[49400:49500], self.u))
        return projects
        ...
        for row_idx in range (0,len(x)):
            for col_idx in range (0,len(x[0])):
                if global_feature_std[col_idx] != 0:
                    x[row_idx][col_idx] = (x[row_idx][col_idx] - global_feature_mean[col_idx])/global_feature_std[col_idx]
                else:
